...
## Quick highlight benchmark

Here we compare MCVO to AnyCam, the method it came out of, and to MCT + AnyCam, the thesis model I also trained. These are the only methods that are truly comparable: trained without any labels, run on any video without training per dataset, and predict the intrinsics as well. All other VO pipelines, supervised, self-supervised and classical, are benchmarked in the [full comparison](#full-comparison) at the bottom of the page.

Same protocol for all three: NVIDIA A40, identical 4-frame windows, CUDA-synchronised, clock measured end to end per call, accuracy as medians over 16 windows per sequence, no bundle adjustment. Datasets: Sintel, TUM-RGBD, KITTI. Highlighted cells are the best in the row.

<p align="center"><picture><source media="(prefers-color-scheme: dark)" srcset="assets/quickview_dark.png"><img src="assets/quickview.png" alt="Quick highlight benchmark: MCVO vs MCT + AnyCam vs AnyCam on latency, peak memory, rotation error, translation direction error and focal length error; best cell per row highlighted" width="88%"></picture></p>

**What it says**

- MCVO is the only one of the three that runs from images alone. AnyCam and the thesis pipeline run a depth and a flow network next to the pose model at test time, which is why they need 5.5 to 11 times the latency and 5 to 7 times the memory.
- Rotation accuracy is the same as AnyCam's, 0.47° against 0.48°. The thesis pipeline is slightly better at 0.43°, at eleven times the cost.
- Intrinsics land in between, 27.5 % focal error against 18 % for the thesis model and 50 % for AnyCam's candidate search.
- Translation direction is where MCVO is behind, 62° against 43° to 47°. It is good on driving video and poor when the camera barely moves, the full table has the per-dataset numbers.
- For scale, the big supervised models further down are about twice as accurate in rotation and use 8 to 14 times the memory.
...
assets/make_figures.py       # regenerates the figures on this page
...
Rows are metrics, columns are methods, grouped by how the method is trained: the self-supervised ones first, then the supervised ones, then classical SLAM. **—** means the method cannot produce that quantity (or it does not apply); **n/m** means not measured here. The columns marked "measured here" were run under one protocol: one process per model on the same NVIDIA A40, identical 4-frame windows (20 per model for cost, 16 per sequence for accuracy), CUDA-synchronised, warm-up excluded, end-to-end per call (images in → poses/intrinsics out, including each model's own preprocessing and, for AnyCam, its depth and flow networks). The last four columns are **as reported by their authors** on other hardware and protocols — listed so the picture is complete. Raw: [`honest_benchmarks/latency_summary.json`](honest_benchmarks/latency_summary.json), `experiments/bench_latency.py`, `honest_benchmarks/{e3prime_final_square336,e3p_calibA_square336,trajectories_e3p,thesis_final_e4_square336,S_*,kfix_*}`.

|  | **MCVO (ours)** | MCT + AnyCam (thesis) | AnyCam (CVPR'25) | Monodepth2 pose net | π³ | VGGT-1B | Depth Anything 3 | DPVO* | FVO / VoT* | ORB-SLAM3* |
|---|---|---|---|---|---|---|---|---|---|---|
| **Training** | self-supervised | self-supervised | self-supervised | self-supervised, per dataset | supervised | supervised | supervised | supervised | supervised | none (classical) |
| Measured here | yes | yes | yes | yes | yes | yes | yes | reported | reported | reported |
| Inputs at test time | images | images (+ depth & flow nets) | images (+ runs depth & flow nets) | image pairs | images | images | images | images + **intrinsics** | images | images + **intrinsics** |
| Parameters | **154 M** (67 M trained) | 460 M (25 M trained) | 115 M incl. depth & flow nets | **13 M** | 959 M | 1257 M | 1690 M | n/m (small) | ~500 M | — (not a network) |
| Weights on disk | 0.57 GiB | 1.71 GiB | 0.43 GiB | **0.05 GiB** | 3.57 GiB | 4.68 GiB | 6.30 GiB | n/m | n/m | — |
| Peak GPU memory, 4-frame window | 0.69 GiB | 5.0 GiB | 3.7 GiB | **0.09 GiB** | 5.5 GiB | 7.0 GiB | 9.7 GiB | ~4.9 GB (3090, streaming) | not published | CPU |
| Latency, 4-frame window (A40) | 75 ms | 820 ms | 413 ms | **13 ms** | 171 ms | 203 ms | 600 ms | ~60 fps @512×384 (3090); 120 fps variant | "~2× DPVO", "10× 3D foundation models" (3090) | real-time, CPU |
| Latency, 8-frame window | 132 ms | 1645 ms | 877 ms | **30 ms** | 300 ms | 376 ms | 1180 ms | n/m | n/m | n/m |
| Rotation error, median — Sintel / TUM / KITTI | 0.48° / 0.76° / 0.18° | 0.40° / 0.67° / 0.23° | 0.50° / 0.74° / 0.20° | 0.80° / 0.77° / 0.30° | 0.22° / 0.26° / 0.11° | 0.28° / 0.32° / 0.12° | 0.19° / 0.27° / 0.09° | n/m | n/m | n/m |
| Heading error, KITTI (zero-shot for ours) | 5.1° | 28.2° | 28.6° | 1.2° (trained on KITTI) | 2.2° | 4.6° | 1.3° | n/m | n/m | n/m |
| Heading error — Sintel / TUM | 104° / 75° | 47° / 65° | 49° / 50° | 57° / 86° | 27° / 34° | 38° / 37° | 19° / 32° | n/m | n/m | n/m |
| Focal error — Sintel / TUM / KITTI | 31.1 % / 14.3 % / 37.1 % (distilled head) | 20.7 % / 12.9 % / 20.4 % | 70.3 % / 14.6 % / 66.9 % | — (pose only) | 25.2 % / 7.6 % / 28.9 % | 34.0 % / 25.8 % / 37.1 % | 24.4 % / 4.6 % / 15.8 % | — (needs intrinsics as input) | — (pose only) | — (needs intrinsics as input) |
| Trajectory (Sintel ATE, Sim3; 8-frame windows chained, no BA) | 0.29 | 0.18 | 0.10 | n/m | n/m | n/m | n/m | n/m (strong: BA inside) | n/m | n/m (strong) |
| Source | this repo | this repo | [paper](https://arxiv.org/abs/2503.23282) | measured here, `mono_640x192` weights of [Godard 2019](https://arxiv.org/abs/1806.01260) | [paper](https://arxiv.org/abs/2507.13347) | [paper](https://arxiv.org/abs/2503.11651) | [paper](https://arxiv.org/abs/2511.10647) | [Teed 2023](https://proceedings.neurips.cc/paper_files/paper/2023/file/7ac484b0f1a1719ad5be9aa8c8455fbb-Paper-Conference.pdf) | [Yugay 2025](https://arxiv.org/abs/2510.03348) | [Campos 2021](https://arxiv.org/abs/2007.11898) |
